...
def inject_repo_file_loader(output_dir):
    ...
    for path in Path(output_dir).rglob("*worker*.js"):
        ...
        injection = (
            ...
            # Monkey-patch builtins.open and io.open so that relative
            # paths that fail in the current CWD get resolved against
            # the original CWD where data files were written.  This
            # handles CWD changes made at the C level (emscripten) that
            # bypass Python's os.chdir.
            "+'print(\\x22[marimo-data] base CWD:\\x22,_db)\\n'"
            "+'import builtins as _bi,io as _io\\n'"
            "+'_oo=_bi.open\\n'"
            "+'def _fo(file,*a,**k):\\n'"
            "+' try:return _oo(file,*a,**k)\\n'"
            "+' except FileNotFoundError:\\n'"
            "+'  _fs=str(file)\\n'"
            "+'  if not os.path.isabs(_fs):\\n'"
            "+'   _alt=os.path.join(_db,_fs)\\n'"
            "+'   if os.path.exists(_alt):\\n'"
            "+'    print(\\x22[marimo-data] resolved\\x22,_fs,\\x22->\\x22,_alt)\\n'"
            "+'    _k2={_kk:_kv for _kk,_kv in k.items() if _kk!=\\x22opener\\x22}\\n'"
            "+'    return _oo(_alt,*a,**_k2)\\n'"
            "+'  raise\\n'"
            "+'_bi.open=_fo\\n'"
            "+'_io.open=_fo\\n'"
